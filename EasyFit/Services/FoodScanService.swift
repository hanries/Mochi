...
enum ScanServiceFactory {
    // ← set useMock = false once you've added your Anthropic API key below
    static let useMock = false

    private static let anthropicKey = "YOUR_ANTHROPIC_API_KEY"  // ← paste your key here

    static func make() -> any FoodScanServiceProtocol {
        useMock || anthropicKey == "YOUR_ANTHROPIC_API_KEY"
            ? MockFoodScanService()
            : FoodScanService(apiKey: anthropicKey)
    }
}
